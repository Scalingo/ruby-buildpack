  ...
  def self.warn_bundler_upgrade(metadata:, bundler_version:)
    ...
    if old_bundler_version && old_bundler_version != bundler_version
      warn(<<~WARNING, inline: true)
        ...
        If you see problems related to the new bundler version, you can revert
        to the previous version by updating the `BUNDLED WITH` value in your
        `Gemfile.lock`. For example:
  ...
  def set_default_web_concurrency
    ...
    <<~EOF
      ...
      *)
      ...
      export WEB_CONCURRENCY_SET_BY="scalingo/ruby"
    ...
    if ruby_version.default?
      warn(<<~WARNING)
        ...
        For more information see:
          https://doc.scalingo.com/languages/ruby
      WARNING
    end
    ...
    if $?.success?
      ...
      if /An error occurred while installing sqlite3/.match?(bundler_output)
        error_message += <<~ERROR
          ...
          https://doc.scalingo.com/databases/sqlite
        ERROR
      end
